...
function compute(prices, spikes, inst) {
  ...
  // ── Layer 1: Composite zone (primary driver) ──
  if      (pred.composite >= 80) { score += spikeBull ? 9 : -9;   confs.push({ t: `🔥 FIRE ZONE — Composite ${pred.composite}% — SPIKE IMMINENT`, b: spikeBull }) }
  else if (pred.composite >= 65) { score += spikeBull ? 6 : -6;   confs.push({ t: `⚡ HOT — Composite ${pred.composite}%`, b: spikeBull }) }
  else if (pred.composite >= 50) { score += spikeBull ? 4 : -4;   confs.push({ t: `WARM — Composite ${pred.composite}%`, b: spikeBull }) }
  else if (pred.composite >= 30) { score += spikeBull ? 2 : -2;   confs.push({ t: `BUILDING — Composite ${pred.composite}%`, b: spikeBull }) }

  // ── Layer 2: Sub-signals now SCORE (not just display) ──
  if (pred.compressed) {
    score += spikeBull ? 2 : -2
    confs.push({ t: `Volatility compressed (${(pred.atrS/pred.atrL*100).toFixed(0)}% of avg) — pre-spike squeeze`, b: spikeBull })
  }
  if (pred.driftScore >= 60) {
    score += spikeBull ? 1.5 : -1.5
    confs.push({ t: `Strong drift ${pred.driftScore}% toward spike direction`, b: spikeBull })
  }
  if (pred.meanScore >= 40) {
    score += spikeBull ? 1.5 : -1.5
    confs.push({ t: `Mean reversion stretch z=${pred.zScore}`, b: spikeBull })
  }
  if (pred.exhaustScore >= 48) {
    score += spikeBull ? 1.5 : -1.5
    confs.push({ t: `Counter-move exhaustion (${pred.exhaustScore}%)`, b: spikeBull })
  }

  // ── Layer 3: Geometric readiness (ticks since spike vs avg) ──
  if (pred.geo >= 85) {
    score += spikeBull ? 2 : -2
    confs.push({ t: `Geometric probability ${pred.geo}% — overdue for spike`, b: spikeBull })
  } else if (pred.geo >= 70) {
    score += spikeBull ? 1 : -1
    confs.push({ t: `Geometric probability ${pred.geo}%`, b: spikeBull })
  }

  // ── Layer 4: Pre-spike alert flag (3+ layers hot) ──
  if (pred.alert) {
    score += spikeBull ? 2 : -2
    confs.push({ t: `⚡ PRE-SPIKE ALERT — ${pred.layersHot}/5 predictor layers firing`, b: spikeBull })
  }

  // ── Layer 5: Post-spike retrace entry (FIXED — was backwards) ──
  if (ps) {
    // After a Boom spike UP → price retraces DOWN → BUY the dip (isBoom = true = spikeBull)
    // After a Crash spike DOWN → price bounces UP → SELL the bounce (isBoom = false)
    if (ps.q === 'PRIME') { score += spikeBull ? 3 : -3; confs.push({ t: `PRIME post-spike retrace (${ps.ago} ticks, ${Math.abs(ps.ret).toFixed(3)}% retrace)`, b: spikeBull }) }
    else if (ps.q === 'GOOD') { score += spikeBull ? 1.5 : -1.5; confs.push({ t: `GOOD post-spike retrace (${ps.ago} ticks)`, b: spikeBull }) }
  }

  // ── Layer 6: Market structure ──
  if (ms.trend === 'BULLISH') { score += 1; confs.push({ t: 'Bullish structure HH+HL', b: true }) }
  if (ms.trend === 'BEARISH') { score -= 1; confs.push({ t: 'Bearish structure LH+LL', b: false }) }
  if (ms.bos && ms.trend === 'BULLISH') { score += 0.5; confs.push({ t: 'Bullish BOS confirmed', b: true }) }
  if (ms.bos && ms.trend === 'BEARISH') { score -= 0.5; confs.push({ t: 'Bearish BOS confirmed', b: false }) }
  ...
  // ── Normalise: max possible score is ~22, scale conf to % ──
  const conf = Math.min(97, Math.round((Math.abs(score) / 22) * 100))
  // BUY threshold lowered: composite 50%+ with 2+ supporting signals = act
  const dir  = score >= 3.5 ? 'BUY' : score <= -3.5 ? 'SELL' : 'WAIT'
  ...
  // ── EXIT WARNING ENGINE ────────────────────────────────────────────────
  // Detects when a trade entered on a BUY/SELL signal should be abandoned
  // because the spike setup has collapsed. Returns exitWarning object.
  const exitReasons = []

  // 1. Composite collapsed — setup energy gone
  if (pred.composite < 25) {
    exitReasons.push(`Composite collapsed to ${pred.composite}% — spike setup void`)
  }

  // 2. Zone degraded badly (WARM or above → now EARLY or BUILDING)
  if (pred.zone === 'EARLY') {
    exitReasons.push(`Zone dropped to EARLY — setup fully reset`)
  }

  // 3. Wrong-direction spike fired — counter-spike resets the cycle
  // A wrong spike means: for Boom, a DOWN spike fired; for Crash, an UP spike fired
  if (spikes.length > 0) {
    const lastSpike = spikes[spikes.length - 1]
    const wrongDir = isBoom ? lastSpike.dir === 'DOWN' : lastSpike.dir === 'UP'
    const recent = (n - 1 - lastSpike.idx) < 15  // fired in last 15 ticks
    if (wrongDir && recent) {
      exitReasons.push(`Wrong-direction spike fired ${n - 1 - lastSpike.idx} ticks ago — cycle reset`)
    }
  }

  // 4. Geo probability low AND ticks since spike is low (spike just fired, too early to re-enter)
  if (pred.since < Math.round(inst.avg * 0.15) && pred.geo < 20) {
    exitReasons.push(`Only ${pred.since} ticks since last spike — too early, wait for setup to build`)
  }

  // 5. Score has flipped to opposing direction — market moved against trade
  const tradeFlipped = (dir === 'SELL' || dir === 'WAIT') && score < -1
  if (tradeFlipped && exitReasons.length === 0) {
    exitReasons.push(`Signal flipped — momentum no longer supports the spike direction`)
  }

  const exitWarning = exitReasons.length > 0
    ? { active: true, reasons: exitReasons, urgency: exitReasons.length >= 2 ? 'URGENT' : 'CAUTION' }
    : { active: false, reasons: [], urgency: null }
